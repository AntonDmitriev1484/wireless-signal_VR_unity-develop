...
# -----------------------------------------------------------------------------
# Through-wall (transmission) detection and rooms (v3)
# -----------------------------------------------------------------------------
COLLINEAR_COS = 0.9999   # a transmission through Sionna's thin slab continues straight


def n_transmissions(points):
    """Number of interaction vertices where the ray continues straight
    (= wall/object transmissions; reflections change direction)."""
...
def is_through_wall(row):
    return int(row["Total_Interactions_for_Path"]) >= 1 and n_transmissions(row_points(row)) > 0


def room_of(xy):
    """Room label of an xy point: office (x < 0, incl. the alcove), living
    room (x >= 0, y < 4) or kitchen (x >= 0, y >= 4)."""
    x, y = float(xy[0]), float(xy[1])
    if x < 0.0:
        return "office"
    return "kitchen" if y >= 4.0 else "living"


def select_final_rows(powerfiltered_csv, raw_csv, out_csv, tx=None, rx=None, max_rows=MAX_PATH_ROWS,
                      similarity_threshold=GEOMETRY_SIMILARITY, box_corners=None, n_samples=20):
    """Pick at most `max_rows` rows for Unity (v3 rule):
      * the LoS row (if any) and the strongest reflection-only cabinet-bounce
        row are always kept (from the raw export, even when weak);
      * if Tx and Rx are in different rooms, exactly ONE through-wall
        (transmission) row is kept - the strongest one;
      * the remaining slots are filled strongest-first with reflection-only
        paths whose geometry differs from everything already selected.
    Through-wall rows are otherwise excluded (same-room links get none).
    Rows are copied verbatim; the schema is untouched."""
    ...
    thru = {r["Interaction_Coordinates"] for r in src if is_through_wall(r)}

    def reflection_only(r):
        return r["Interaction_Coordinates"] not in thru

    must = [r for r in src if int(r["Total_Interactions_for_Path"]) == 0]
    if box_corners is not None:
        bounce = [r for r in src if int(r["Total_Interactions_for_Path"]) >= 1 and reflection_only(r) and
                  any(point_in_box_xy(p, box_corners) for p in row_bounces(r))]
        if bounce:
            must.append(max(bounce, key=power))
    cross_room = tx is not None and rx is not None and room_of(tx) != room_of(rx)
    if cross_room:
        cand = [r for r in pf if not reflection_only(r)] or [r for r in src if not reflection_only(r)]
        if cand:
            must.append(max(cand, key=power))
    ...
    for r in sorted(pf, key=lambda r: -power(r)):
        if len(selected) >= max_rows:
            break
        if r["Interaction_Coordinates"] in keys or not reflection_only(r):
            continue
        f = feat(r)
...
def analyse_rows_v3(rows, box_corners=None):
    info = analyse_rows(rows, box_corners)
    info["n_through_wall"] = int(sum(is_through_wall(r) for r in rows))
    return info
...
def run_condition(name, xml_path, tx, rx, work_dir, final_dir, box_corners=None,
    ...
    select_final_rows(pf_csv, raw_csv, fin_csv, tx=tx, rx=rx, box_corners=box_corners)
    ...
    metrics = {
        ...
        "raw": analyse_rows_v3(raw_rows, box_corners),
        "final": analyse_rows_v3(fin_rows, box_corners),
...
def reselect_condition(name, tx, rx, work_dir, final_dir, box_corners=None):
    """v3: rebuild only the final path CSVs (Sionna intermediate + Unity file)
    from the existing raw / power-filtered exports with the v3 selection rule.
    No Sionna call; heatmap, RSS and every other file stay untouched.
    Returns {"raw": ..., "final": ...} analyses."""
    work_dir = Path(work_dir); final_dir = Path(final_dir)
    raw_csv = work_dir / f"{name}_paths_raw_sionna.csv"
    pf_csv = work_dir / f"{name}_paths_powerfiltered_sionna.csv"
    fin_csv = work_dir / f"{name}_paths_final_sionna.csv"
    select_final_rows(pf_csv, raw_csv, fin_csv, tx=tx, rx=rx, box_corners=box_corners)
    utils.transform_csv_for_unity(str(fin_csv), str(final_dir / f"{name}.csv"))
    _, raw_rows = read_rows(raw_csv)
    _, fin_rows = read_rows(fin_csv)
    return {"raw": analyse_rows_v3(raw_rows, box_corners), "final": analyse_rows_v3(fin_rows, box_corners)}
