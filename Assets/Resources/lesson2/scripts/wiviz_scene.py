...
# -----------------------------------------------------------------------------
# Mesh-level probes (lesson 2): geometric LoS and open-floor checks.
# Pure numpy Moller-Trumbore; trimesh's ray helpers need rtree/embree.
# -----------------------------------------------------------------------------
def _segment_hits_triangles(tri, p, q):
    ...
    qv = np.cross(s, e1)
    v = f * np.einsum('j,ij->i', d, qv)
    t = f * np.einsum('ij,ij->i', e2, qv)
    return bool(np.any(ok & (u >= 0) & (v >= 0) & (u + v <= 1) & (t > 1e-6) & (t < 1 - 1e-6)))


def los_blockers(p, q, base_xml=BASE_XML):
    """Names of the base-scene shapes whose triangles cut the 3D segment p->q."""
    out = []
    for name, verts, faces in _transformed_shapes(base_xml):
        if _segment_hits_triangles(verts[faces], p, q):
            out.append(name)
    return out


def on_open_floor(pt_xy, base_xml=BASE_XML, z_max=1.5):
    """Names of furniture shapes whose actual geometry occupies the vertical
    column above pt_xy up to z_max (empty = open floor).  Mesh-accurate: a
    bounding-box footprint would flag the whole kitchen because the L-shaped
    counter mesh's bbox spans the room."""
    p = (float(pt_xy[0]), float(pt_xy[1]), 0.02)
    q = (float(pt_xy[0]), float(pt_xy[1]), float(z_max))
    out = []
    for name, verts, faces in _transformed_shapes(base_xml):
        if "Floor_plan" in name:
            continue
