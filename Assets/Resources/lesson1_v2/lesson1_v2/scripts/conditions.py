...
CAB_SIZE = (1.2, 0.4, 2.2)            # metal cabinet: width x depth x height (m)
...
def cab(center, yaw_deg):
    return dict(name="Metal_Cabinet", size=CAB_SIZE, center=tuple(float(v) for v in center),
...
# -----------------------------------------------------------------------------
# los_creation: router + TV fixed with LoS; a metal cabinet goes to one of 4
# spots.  Three spots stand on the direct line (staggered left/right of it and
# at different distances from the TV), one stands beside the TV and keeps LoS.
# -----------------------------------------------------------------------------
TVK = dict(rx_label="TV", rss_point=TV_RSS_POINT, rss_mode="rx_height", floor_point=TV_FLOOR_POINT)
# Set 1 - router on the living-room desk (line TV->router runs NNE, over the couch).
add("los_creation", 1, "A", TX_LIVING_DESK, TV, [cab((5.75, -5.65), 0.0)], note="on the direct line, 2.0 m from the TV, shifted right - blocks LoS", **TVK)
add("los_creation", 1, "B", TX_LIVING_DESK, TV, [cab((5.00, -6.30), 0.0)], note="on the direct line, 1.3 m in front of the TV - blocks LoS", **TVK)
add("los_creation", 1, "C", TX_LIVING_DESK, TV, [cab((1.50, -7.50), 0.0)], note="beside the TV stand along the south wall (west) - LoS kept", **TVK)
add("los_creation", 1, "D", TX_LIVING_DESK, TV, [cab((5.30, -5.00), 0.0)], note="on the direct line, just in front of the couch - blocks LoS", **TVK)
# Set 2 - router on the wall shelf (line TV->router runs WNW, along the TV stand).
add("los_creation", 2, "A", TX_SHELF, TV, [cab((6.90, -7.50), 0.0)],  note="beside the TV stand along the south wall (east) - LoS kept", **TVK)
add("los_creation", 2, "B", TX_SHELF, TV, [cab((2.99, -6.48), 0.0)],  note="on the direct line, 2.3 m from the TV - blocks LoS", **TVK)
add("los_creation", 2, "C", TX_SHELF, TV, [cab((2.12, -6.00), 60.9)], note="on the direct line, 3.3 m from the TV - blocks LoS", **TVK)
add("los_creation", 2, "D", TX_SHELF, TV, [cab((1.24, -5.51), 60.9)], note="on the direct line, 4.3 m from the TV (near the shelf) - blocks LoS", **TVK)
