...
# -----------------------------------------------------------------------------
# Through-wall (transmission) detection and rooms (v2)
# -----------------------------------------------------------------------------
...
def analyse_rows_by_rx(rows):
    """analyse_rows per Rx group (+ through-wall count), keyed by Rx_Number."""
    groups = {}
    for r in rows:
        groups.setdefault(r["Rx_Number"], []).append(r)
    out = {}
    for k, v in sorted(groups.items(), key=lambda kv: int(kv[0])):
        info = analyse_rows(v)
        info["n_through_wall"] = int(sum(is_through_wall(r) for r in v))
        out[k] = info
    return out


def select_final_rows(powerfiltered_csv, raw_csv, out_csv, tx=None, rxs=None, max_rows_by_rx=None,
                      similarity_threshold=GEOMETRY_SIMILARITY, n_samples=20):
    """Per receiver, pick at most its ray budget (max_rows_by_rx[rx_number],
    default MAX_PATH_ROWS), v2 rule:
      * the LoS row first (from the raw export, even when weak);
      * if the router and this receiver are in different rooms, exactly ONE
        through-wall (transmission) row - the strongest one;
      * then the strongest remaining reflection-only paths whose geometry
        differs from everything already selected for that receiver.
    ...
    for rx_num in sorted(src_by_rx):
        ...
        rows_src, rows_pf = src_by_rx[rx_num], pf_by_rx.get(rx_num, [])
        thru = {r["Interaction_Coordinates"] for r in rows_src if is_through_wall(r)}
        cross_room = (tx is not None and rxs is not None and
                      room_of(tx) != room_of(rxs[rx_num - 1]))
        ...
        for r in rows_src:
            if int(r["Total_Interactions_for_Path"]) == 0 and r["Interaction_Coordinates"] not in keys:
                take(r)
        if cross_room:
            cand = [r for r in rows_pf if r["Interaction_Coordinates"] in thru] or \
                   [r for r in rows_src if r["Interaction_Coordinates"] in thru]
            if cand:
                best = max(cand, key=power)
                if best["Interaction_Coordinates"] not in keys:
                    take(best)
        for r in sorted(rows_pf, key=lambda r: -power(r)):
            if len(selected) >= cap:
                break
            if r["Interaction_Coordinates"] in keys or r["Interaction_Coordinates"] in thru:
                continue
            f = feat(r)
...
def run_condition(name, xml_path, tx, rxs, work_dir, final_dir, heatmap=None, verbose=True):
    ...
    select_final_rows(pf_csv, raw_csv, fin_csv, tx=tx, rxs=rxs, max_rows_by_rx=ray_caps)
...
def reselect_condition(name, tx, rxs, rss_rx_dbm, work_dir, final_dir):
    """v2: rebuild only the final path CSVs (Sionna intermediate + Unity file)
    from the existing raw / power-filtered exports with the v2 selection rule.
    No Sionna call; heatmap and RSS stay untouched (rss_rx_dbm from the stored
    metrics sets the per-receiver ray budgets).  Returns {"raw", "final", "ray_caps"}."""
    work_dir = Path(work_dir); final_dir = Path(final_dir)
    raw_csv = work_dir / f"{name}_paths_raw_sionna.csv"
    pf_csv = work_dir / f"{name}_paths_powerfiltered_sionna.csv"
    fin_csv = work_dir / f"{name}_paths_final_sionna.csv"
    ray_caps = {i + 1: rays_for_rss(float(v)) for i, v in enumerate(rss_rx_dbm)}
    select_final_rows(pf_csv, raw_csv, fin_csv, tx=tx, rxs=rxs, max_rows_by_rx=ray_caps)
    utils.transform_csv_for_unity(str(fin_csv), str(final_dir / f"{name}.csv"))
    _, raw_rows = read_rows(raw_csv)
    _, fin_rows = read_rows(fin_csv)
    return {"raw": analyse_rows_by_rx(raw_rows), "final": analyse_rows_by_rx(fin_rows),
            "ray_caps": {str(k): int(v) for k, v in ray_caps.items()}}
