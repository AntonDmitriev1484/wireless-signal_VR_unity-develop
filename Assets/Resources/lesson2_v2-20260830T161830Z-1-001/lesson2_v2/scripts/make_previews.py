...
for ax, c in zip(axes.ravel(), CONDITIONS):
    ...
    for rx_num, rel, n_int, pts in sorted(load_paths(name), key=lambda r: r[1]):
        los = n_int == 0
        if (not los) and n_transmissions(pts) > 0:
...
handles = [Line2D([], [], marker="*", color=ROUTER, ls="", ms=14, label="router (Tx, fixed)"),
           ...
           Line2D([], [], color=THRU, lw=2.0, ls=(0, (4, 2)), label="through-wall ray (one per cross-room phone)")]
...
fig.suptitle("lesson 2 v2 · communication & interference · correct spatial option: D (kitchen)",
